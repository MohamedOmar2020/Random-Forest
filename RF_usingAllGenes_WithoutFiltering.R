...
### Using: ALL genes without filtering
#################################################################################
...
set.seed(333)
rf_output <- randomForest(x =predictor_data, y=target,importance = TRUE, ntree = 1001, mtry =52 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
print(rf_output)
plot(rf_output)
...
pdf(file="varimp_AllGns_NoFilt.pdf")
varImpPlot(rf_output, type=2, n.var=30, scale=FALSE, main="Variable Importance (Gini) for top 50 predictors")
dev.off()

## An MDS plot provides a sense of the separation of classes.
pdf(file="MDS_AllGns_NoFilt.pdf")
...
pdf(file="ROC_train_AllGns_NoFilt.pdf")
plot(perf_ROC_train, main="ROC plot train AllGns No Filtering")
...
pdf(file="ROC_test_AllGns_NoFilt.pdf")
plot(perf_ROC_test, main="ROC plot All Gns No Filtering")
text(0.5,0.5,paste("AUC = ",format(AUC_test, digits=5, scientific=FALSE)))
dev.off()

#########
## Save RF classifier
save(rf_output, file = "./Objs/RF_classifier_AllGns_NoFilt.rda")

## Save RF importances
save(rf_importances, file = "./Objs/RF_importances_AllGns_NoFilt.rda")
