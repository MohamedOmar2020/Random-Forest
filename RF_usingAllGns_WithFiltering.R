...
### Using: ALL genes with filtering according to following criteria: (1) At least 10% of samples should have raw intensity greater than 80; (2) The coefficient of variation (sd/mean) is between 0.7 and 10
#################################################################################
...
## We will first take the values and un-log2 them, then filter out any genes according to following criteria: (1) At least 10% of samples should have raw intensity greater than 80; (2) The coefficient of variation (sd/mean) is between 0.7 and 10.
X <- usedTrainMat
ffun <- filterfun(pOverA(p = 0.1, A = 80), cv(a = 0.7, b = 10))
...
predictor_names <- c(as.vector(rownames(filt_Data))) #gene symbol
...
set.seed(333)
rf_output <- randomForest(x =predictor_data, y=target,importance = TRUE, ntree = 1001, mtry =39 ,proximity=TRUE, na.action = na.omit, sampsize = sampsizes)
print(rf_output)
plot(rf_output)
...
pdf(file="varimp_AllGns_Filt.pdf")
...
pdf(file="ROC_train_AllGns_Filt.pdf")
plot(perf_ROC_train, main="ROC plot train AllGns with Filtering")
...
pdf(file="ROC_test_AllGns_Filt.pdf")
plot(perf_ROC_test, main="ROC plot All Gns with Filtering")
text(0.5,0.5,paste("AUC = ",format(AUC_test, digits=5, scientific=FALSE)))
dev.off()

#########
## Save RF classifier
save(rf_output, file = "./Objs/RF_classifier_AllGns_Filt.rda")

## Save RF importances
save(rf_importances, file = "./Objs/RF_importances_AllGns_Filt.rda")
